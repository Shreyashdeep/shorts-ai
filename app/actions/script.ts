...
export const generateScript = async (part: string) => {
    ...
    // const prompt = `Write a script to generate a 30 seconds video on topic: "${part}" in a sarcastic and meme-style tone. For each scene, create an AI image prompt in realistic format and a matching sarcastic/meme-like content text. Give the result in JSON format with imagePrompt and contentText as fields, inside an array named "content". Do not include any extra labels or responses like "scene1" etc. Just provide exactly what is asked, nothing more.`
    // console.log(prompt);
